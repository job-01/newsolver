...
impl PostFlopGame {
    ...
    /// If the current node is a chance node, returns a list of cards that can be dealt, restricted to allowed runouts.
    ///
    ...
    pub fn possible_cards(&self) -> u64 {
        ...
        // Existing logic for determining dead cards (no bunching)
        ...
        // Existing bunching logic remains unchanged (handled similarly)
        else {
        ...
        // Apply allowed_cards restriction
        let allowed_mask = self.allowed_cards.as_ref().map_or(((1 << 52) - 1) ^ dead_mask, |cards| {
            cards.iter().fold(0u64, |acc, &card| acc | (1 << card))
        });

        ((1 << 52) - 1) ^ dead_mask & allowed_mask // Intersect with allowed cards
    }
    ...
    pub fn play(&mut self, action: usize) {
        ...
        if self.is_chance_node() {
            ...
            let actual_card = if action == usize::MAX {
                // Find the lowest valid card from possible_cards, respecting allowed_cards
                let possible = self.possible_cards();
                let mut card = possible.trailing_zeros() as Card;
                while card < 52 && (possible & (1 << card)) == 0 {
                    card += 1;
                }
                if card >= 52 {
                    panic!("No valid card found in possible_cards");
                }
                card
            } else {
                action as Card
            ...
            // Validate against allowed_cards
            if let Some(ref allowed) = self.allowed_cards {
                if !allowed.contains(&actual_card) {
                    panic!("Card {} not in allowed runouts", actual_card);
                }
            }

            // Swap the suit if swapping was performed in turn (existing logic)
            ...
            // Find the action index from isomorphic chances (existing logic)
    ...
    pub fn set_allowed_runouts(&mut self, allowed_cards: &[u8]) -> Result<(), String> {
        ...
        // Validate cards are within bounds and not duplicates
        let mut seen = [false; 52];
        for &card in allowed_cards {
            if card >= 52 {
                return Err(format!("Invalid card index: {}", card));
            }
            if seen[card as usize] {
                return Err(format!("Duplicate card in allowed runouts: {}", card));
            }
        ...
        for &card in allowed_cards {
            if (1 << card) & flop_mask != 0 {
                return Err(format!("Card {} overlaps with flop", card));
            }
        }

        self.allowed_cards = Some(allowed_cards.to_vec());
        Ok(())
    }
}